...
def oil_prod(variables_values = [0, 0, 0]):
    ...
    a=variables_values[1]
        
    exc = ((x+1)/2*(x+1)/np.tan(a)+(x+1)*50)*20
    y=variables_values[2]
    dew = ((y+1)/2*(y+1)/np.tan(a)+(y+1)*50)*20   

    func_value = abs(income - exc - dew)
    return func_value

gwo = grey_wolf_optimizer(pack_size = 15, min_values = [100,10,100], max_values = [1500,45,1500], iterations = 100, target_function = oil_prod)
